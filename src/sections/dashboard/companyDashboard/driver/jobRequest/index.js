...
import ApplyViewJob from "@/module/dashboard/companyCard/viewJob";
import ApplyJobEditModal from "@/module/dashboard/companyCard/viewEditJob";
import axiosInstance from "@/utils/axios";
import SkeletonLoader from "@/components/skeleton";
import { JobSekelton } from "@/components/not-found";
import { useFormik } from "formik";
...
const DashboardJobRequest = () => {
  ...
  console.log('datasdatasdatas',data) 
  ...
  useEffect(() => {
    // if (driverId && userType) {
      dispatch(getDriver({ user_id: driverId, type: userType, lat: 0, long: 0 }));
    // }
  }, []);
              ...
              {data && data?.length > 0 ? (
                data.map((item, index) => {
                  ...
                  let addressDetail =
                    ...
                    let request_id;
                    let job_requests_assign = false
                    let job_requests_data;
