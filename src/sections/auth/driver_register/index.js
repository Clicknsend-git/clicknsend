  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const responses = await Promise.all([
        axios.get("https://evsexpres.com/api/auth/default/vehicle-body", {
          params: { type: "Vans" },
        }),
        axios.get("https://evsexpres.com/api/auth/default/vehicle-body", {
          params: { type: "Trucks/lorrys" },
        }),
      ]);

      const combinedData = responses
        .flatMap((response) => response.data?.view_data || [])
        .map((item) => ({
          label: item,
          value: item.toLowerCase().replace(/ /g, "_"),
        }));

      setVehicleOptions(combinedData);
    } catch (error) {
